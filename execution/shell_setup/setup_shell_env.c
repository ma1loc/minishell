...
t_setup  *init_setup_struct(t_gc *gc)	// done gc
{
    t_setup  *setup;
...
// the check this later
int update_key_value(t_env *current, char *key, char *value, t_gc *gc)
{
    while (current)
    {
        if (ft_strcmp(current->key, key) == 0)
        {
            ...
            current->value = ft_strdup(value, gc);	// >>> use gc
            if (!current->value)
...
//     else
//     {
//         status = update_key_value(setup->env, key, value, gc);
//         if (status == 1) // >>> key not found
//             status = export_key_only(setup, key, gc);
//     }
// }
...
t_setup	*shell_env_setup(char **env, t_gc *gc)
{
    ...
    update_env(setup, "OLDPWD", setup->pwd, gc);	// to check later on
